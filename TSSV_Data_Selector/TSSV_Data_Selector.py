...
def select_file_from_timestamp(time_in, extension='.flac', file_interval=5):
    ...
    file_minute = start_time.minute - (start_time.minute % file_interval)
    file_time = start_time.replace(minute=file_minute, second=0, microsecond=0)
...
def extract_audio_section(start_time, duration_ms, file_interval=5):

    # Open the first audio file
    audio_file = sf.SoundFile(select_file_from_timestamp(start_time))
